...
class RegistroAsistencia(wx.Frame):
    ...
    def InitUI(self):
        ...
        self.SetTitle('Registro de Asistencia')
        self.Maximize(True)
        self.Centre()
    ...
    def UpdateGrid(self):
        ...
        asistencia_headers = ["ASISTENCIA", "% DE ASISTENCIA", "FALTAS"]
        ...
        for row, nombre in enumerate(self.nombres_registrados, start=4):
            ...
            for col, estado in enumerate(estado_asistencia, start=1):
                ...
                elif fecha_actual < self.today and not estado:  # Falta (sin registro)
                    estado_asistencia[dia_actual - 1] = 'X'
            ...
            porcentaje_asistencia = (asistencia / num_dias_mes) * 100 if num_dias_mes > 0 else 0
    ...
    def OnWeeklyReport(self, event):
        # Crear y mostrar la ventana del reporte semanal
        weekly_report_frame = WeeklyReportFrame(self)
        weekly_report_frame.Show()

class WeeklyReportFrame(wx.Frame):
    def __init__(self, parent):
        super().__init__(parent, title="Reporte Semanal", size=(800, 600))
    ...
    def UpdateWeeklyGrid(self):
        ...
        total_cols = 7 + 2  # 7 días de la semana + columnas adicionales
        ...
        for row, nombre in enumerate(self.parent.nombres_registrados, start=3):
            ...
            for col, estado in enumerate(estado_asistencia, start=1):
                if estado == '✓':
                    self.grid.SetCellValue(row, col, estado)
                    self.grid.SetCellBackgroundColour(row, col, wx.Colour(0, 255, 0))
                elif not estado:  # Falta (sin registro)
                    self.grid.SetCellValue(row, col, 'X')
                    self.grid.SetCellBackgroundColour(row, col, wx.Colour(255, 0, 0))
